...
CalDim = 30;
CalLen = 100;
CalLoc = OB.HeadWid/2 + CalDim/2;
RelCalDim = CalDim/(PROJdgn.fov*SS);
RelCalLen = CalLen/(PROJdgn.fov*SS);
RelCalLoc = CalLoc/(PROJdgn.fov*SS);

%--------------------------------------
% CalTube1
%--------------------------------------
MatCalDim = M*RelCalDim;
MatCalBot = (M+1)/2 - M*RelCalLen/2;
MatCalTop = (M+1)/2 + M*RelCalLen/2;
CX = (M+1)/2 + M*RelCalLoc;
CY = (M+1)/2;
for x = 1:M
    for y = 1:M
        r = sqrt((x-CX)^2 + (y-CY)^2);
        if r < MatCalDim/2
            Ob(MatCalBot:MatCalTop,x,y) = 1.0;
        end
    end
end
