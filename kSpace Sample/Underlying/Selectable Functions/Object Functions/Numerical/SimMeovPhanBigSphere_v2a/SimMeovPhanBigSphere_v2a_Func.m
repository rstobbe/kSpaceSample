...
for x = 1:M
    for y = 1:M
        for z = 1:M
            r = sqrt((x-CX)^2 + (y-CY)^2 + (z-CZ)^2);
            if r <= rmax1
                Ob(y-round(SepDist+rmax1+rmax4)+ys,x-round(SepDist+rmax1+rmax2)-xs,z) = 1;         
            end
            if r <= rmax2
                Ob(y-round(SepDist+rmax1+rmax4)+ys,x-xs,z) = 1;         
            end
            if r <= rmax3
                Ob(y-round(SepDist+rmax1+rmax4)+ys,x+round(SepDist+rmax3+rmax2)-xs,z) = 1;           
            end
            if r <= rmax4
                Ob(y+ys,x-round(SepDist+rmax1+rmax2)-xs,z) = 1;         
            end
            if r <= rmax5
                Ob(y+ys,x-xs,z) = 1;         
            end
            if r <= rmax6
                Ob(y+ys,x+round(SepDist+rmax3+rmax2)-xs,z) = 1;         
            end            
            if r <= rmax7
                Ob(y+round(SepDist+rmax7+rmax4)+ys,x-round(SepDist+rmax1+rmax2)-xs,z) = 1;         
            end
            if r <= rmax8
                Ob(y+round(SepDist+rmax7+rmax4)+ys,x-xs,z) = 1;         
            end
            if r <= rmax9
                Ob(y+round(SepDist+rmax7+rmax4)+ys,x+round(SepDist+rmax3+rmax2)-xs,z) = 1;           
            end
        end
    end
end
