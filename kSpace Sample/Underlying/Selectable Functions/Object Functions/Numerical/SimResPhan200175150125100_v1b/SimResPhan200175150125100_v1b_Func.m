...
function [OB,err] = SimResPhan200175150125100_v1b_Func(OB,INPUT)
...
for x = 1:M
    for y = 1:M
        for z = 1:M
            r = sqrt((x-CX)^2 + (y-CY)^2 + (z-CZ)^2);
            if r <= rmax1
                Ob(y-ceil(2*ydist+(rmax1+rmax2)*2)+shift,x+round(rmax1*6),z) = 1;
                Ob(y-ceil(2*ydist+(rmax1+rmax2)*2)+shift,x+round(rmax1*2),z) = 1;
                Ob(y-ceil(2*ydist+(rmax1+rmax2)*2)+shift,x-round(rmax1*2),z) = 1;
                Ob(y-ceil(2*ydist+(rmax1+rmax2)*2)+shift,x-round(rmax1*6),z) = 1;                
            end
            if r <= rmax2
                Ob(y-ceil(ydist+rmax2*2)+shift,x+round(rmax2*6),z) = 1;
                Ob(y-ceil(ydist+rmax2*2)+shift,x+round(rmax2*2),z) = 1;
                Ob(y-ceil(ydist+rmax2*2)+shift,x-round(rmax2*2),z) = 1;
                Ob(y-ceil(ydist+rmax2*2)+shift,x-round(rmax2*6),z) = 1;
            end
            if r <= rmax3
...
OB.Ob = Ob;
OB.ObMatSz = M;
OB.name = 'SimResPhan200175150125100B';
OB.plot = 'CentreSlice';
