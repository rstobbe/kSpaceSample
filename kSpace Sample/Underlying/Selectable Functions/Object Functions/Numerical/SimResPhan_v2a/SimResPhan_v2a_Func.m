...
function [OB,err] = SimResPhan_v2a_Func(OB,INPUT)
...
for x = 1:M
    for y = 1:M
        ...
        if r < rmax
            Ob(y,x,(round(CZ-BotLen/2):round(CZ+BotLen/2))) = 0.8;
        end
    end
end
...
CubeWid = M*0.49;
CubeThk = M*0.15;
Ob(ceil(CX-CubeWid/2):floor(CX+CubeWid/2),ceil(CY-CubeWid/2):floor(CY+CubeWid/2),ceil(CZ-CubeThk/2):floor(CZ+CubeThk/2)) = 0.25;
