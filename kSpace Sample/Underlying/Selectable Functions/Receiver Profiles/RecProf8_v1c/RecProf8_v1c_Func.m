...
function [RPROF,err] = RecProf8_v1c_Func(RPROF,INPUT)
...
for n = 1:sz
    for m = 1:sz
        for p = 1:sz
            rad = sqrt((n-RPROF.cornerinset)^2 + (m-RPROF.cornerinset)^2 + (p-RPROF.cornerinset)^2);
            Prof(n,m,p) = 1/(rad)^RPROF.drofofffact;
            if Prof(n,m,p) > 1
                Prof(n,m,p) = 1;
            end
        end
...
%for n = 1:8
%    figure(1000+n);
%    imshow(squeeze(mOb(:,:,round(0.625*sz),n)),[0 1]);
%    figure(2000+n);
%    imshow(squeeze(Prof(:,:,round(0.625*sz),n)),[0 1]);
%end
...
if plotob == 1
    for n = 1:8
        ...
        IMSTRCT.rows = rows; IMSTRCT.lvl = [minval maxval]; IMSTRCT.SLab = 0; IMSTRCT.figno = 10000+n; 
        IMSTRCT.docolor = 0; IMSTRCT.ColorMap = 'ColorMap4'; IMSTRCT.figsize = [];
        AxialMontage_v2a(testOb,IMSTRCT);
    end
end
