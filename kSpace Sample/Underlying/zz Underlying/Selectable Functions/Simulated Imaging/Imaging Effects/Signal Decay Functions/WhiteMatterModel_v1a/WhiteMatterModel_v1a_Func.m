...
function [SIGDEC,err] = WhiteMatterModel_v1a_Func(SIGDEC,INPUT)

Status2('busy','White Matter signal decay model',3);
...
% Build Model (Myelin)
%---------------------------------------------
load('MyelinGuessDecayFlip60PL180','t','A');                               
ind = find(t==0.18);
t = t(ind:end)-0.18;
Myelin = A(ind:end);
...
% WM Model
%---------------------------------------------
WMdecay = (0.5*Myelin+0.5*ICEC)/100;                        % 50 - 50 model
...
T2decay = interp1(t,WMdecay,samp);
